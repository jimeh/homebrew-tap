...
class UpdateFormulaTest < Minitest::Test
  ...
  def test_updater_changes_only_source_fields_and_removes_bottle
    with_formula("airplan.rb", fixture("formulae/airplan-1.0.0.rb")) do |root, path|
      ...
      assert_equal fixture("formulae/airplan-1.1.0.rb"), File.read(path)
    end
  end

  def test_updater_replay_is_noop
    github = FakeGitHub.new
    with_formula("airplan.rb", fixture("formulae/airplan-1.1.0.rb")) do |root, path|
  ...
  def test_editor_rejects_wrong_repository_and_ambiguous_source_sha
    wrong_repo = fixture("formulae/airplan-1.0.0.rb").sub(
    ...
    ambiguous = fixture("formulae/airplan-1.0.0.rb").sub(
      "  license \"MIT\"",
      %(  sha256 "#{'f' * 64}"\n  license "MIT"),
    )
    assert_raises(HomebrewTap::Automation::Error) do
  ...
  def test_editor_rejects_repository_name_containing_allowlisted_name
    disguised_repo = fixture("formulae/airplan-1.0.0.rb").sub(
